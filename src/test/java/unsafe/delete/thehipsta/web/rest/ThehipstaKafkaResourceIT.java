...
import unsafe.delete.thehipsta.config.KafkaProperties;
...
class ThehipstaKafkaResourceIT {
    ...
    void setup() {
        ...
        ThehipstaKafkaResource kafkaResource = new ThehipstaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/thehipsta-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/thehipsta-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
